...
function displayFeaturedNews() {
	const randomNews = [...featuredNews];

	if (mainNews) {
		const mainNewsIndex = randomNews.findIndex(news => news.title === mainNews.title);
		if (mainNewsIndex !== -1) {
			randomNews.splice(mainNewsIndex, 1);
		}
	}

	const shuffledNews = randomNews.sort(() => Math.random() - 0.5);

	const newsToDisplay = shuffledNews.slice(0, 4);
...
function initializeMainNews() {
	...
	const randomMainNews = featuredNews[randomIndex];

	updateMainNews(randomMainNews);
	mainNews = randomMainNews;
	displayFeaturedNews();
}
...
function autoChangeMainNews() {
	autoChangeTimer = setInterval(() => {
		...
		if (mainNews.title !== randomMainNews.title) {
			updateMainNews(randomMainNews);
			displayFeaturedNews();
		}
	}, 10000);
